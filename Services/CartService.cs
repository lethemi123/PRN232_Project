...
namespace ProjectPRN232.Services
{
    public class CartService
    {
        ...
        public async Task AddToCart(int userId, int variantId, int quantity)
        {
            ...
            var item = await _context.Carts.FirstOrDefaultAsync(c => c.PersonId == userId && c.VariantId == variantId);
        ...
        public async Task<object> GetCart(int userId)
        {
            ...
            return new { items, total, selectedTotal };
        }

        public async Task RemoveItem(int userId, int cartId)
        {
            var item = await _context.Carts.FirstOrDefaultAsync(c => c.CartId == cartId && c.PersonId == userId);
            if (item == null) return;
        ...
        public async Task UpdateQuantity(int userId, int cartId, int quantity)
        {
            ...
            var variant = await _context.ProductVariants.FirstOrDefaultAsync(v => v.VariantId == item.VariantId.Value);
            if (variant == null) throw new Exception("Variant không tồn tại.");
